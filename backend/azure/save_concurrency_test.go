...
func TestBackend_SavePreservesOtherConflicts(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("x-obs-error-code", "LeaseAlreadyPresent")
		w.WriteHeader(409)
		_, _ = io.WriteString(w, "<Error><Code>LeaseAlreadyPresent</Code><Message>conflict</Message></Error>")
	}))
	defer server.Close()
	_, err := New().Save(context.Background(), store.SaveRequest{
		FileRequest: store.FileRequest{Config: store.ContainerConfig{Values: map[string]string{ConnectionStringKey: "AccountName=devstoreaccount1;AccountKey=MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=;BlobEndpoint=" + server.URL + "/devstoreaccount1;", ContainerKey: "archive"}}, Key: "file", FileID: "file"},
...
func TestBackend_SaveNoOverwriteIsAtomic(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		mu.Lock()
		defer mu.Unlock()
		if object != nil && r.Header.Get("If-None-Match") == "*" {
			w.Header().Set("Content-Type", "application/xml")
	...
	for _, body := range []string{"first", "second"} {
		go func() {
			_, err := backend.Save(ctx, store.SaveRequest{
				FileRequest: store.FileRequest{Config: store.ContainerConfig{Values: map[string]string{ConnectionStringKey: "AccountName=devstoreaccount1;AccountKey=MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY=;BlobEndpoint=" + server.URL + "/devstoreaccount1;", ContainerKey: "archive"}}, Key: "same", FileID: "same"},
