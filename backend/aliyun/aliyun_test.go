...
func TestBackendHonorsObjectContractAgainstLocalEndpoint(t *testing.T) {
	...
	if err != nil || parsed.Query().Get("OSSAccessKeyId") != "access" || parsed.Query().Get("Signature") == "" {
		t.Fatalf("AccessURL() = %q, error = %v, want signed OSS URL", accessURL, err)
	}
...
func writeOSSError(w http.ResponseWriter) {
	w.Header().Set("Content-Type", "application/xml")
	w.WriteHeader(http.StatusNotFound)
